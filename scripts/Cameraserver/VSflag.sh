...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "VS flag" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - VS flag found"

else

echo "$Machine_Name - VS flag not found"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,VS_flag,Failed" >> /root/scripts/Cameraserver/Failed_Details/VS_flag.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/VS_flag.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/VS_flag.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/VS_flag.csv
