...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Pendrive" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Pendrive iS Detecting"

else

echo "$Machine_Name - Pendrive Not Detecting"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Pendrive,Failed" >> /root/scripts/Cameraserver/Failed_Details/Pendrive.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Pendrive.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Pendrive.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Pendrive.csv
