...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "BLE Status" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - BLE is Running"

else

echo "$Machine_Name - BLE Not Running"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,BLE_Status,Failed" >> /root/scripts/Cameraserver/Failed_Details/BLE_Status.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/BLE_Status.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/BLE_Status.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/BLE_Status.csv
