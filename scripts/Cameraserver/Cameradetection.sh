...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Camera Detection" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Camera is Detecting"

else

echo "$Machine_Name - Camera Not Detecting"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Camera_Detection,Failed" >> /root/scripts/Cameraserver/Failed_Details/Camera_Detection.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Camera_Detection.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Camera_Detection.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Camera_Detection.csv
