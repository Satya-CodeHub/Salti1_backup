...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Disk Alert" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Disk Usage Lesserthan Threshold"

else

echo "$Machine_Name - Disk Usage Higher Than Threshold"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Disk,Failed" >> /root/scripts/Cameraserver/Failed_Details/Disk_alert.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Disk_alert.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Disk_alert.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Disk_alert.csv
