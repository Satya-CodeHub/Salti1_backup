...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Videos Directory" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Video Directory Present"

else

echo "$Machine_Name - Video Directory Not Present"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Video_Directory,Failed" >> /root/scripts/Cameraserver/Failed_Details/video_directory.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/video_directory.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/video_directory.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/video_directory.csv
