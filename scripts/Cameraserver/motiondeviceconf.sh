...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "motiondeviceconf" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - No Issue in Motion Device Config"

else

echo "$Machine_Name - Issue in MOtion Device Config"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Motion_Device,Failed" >> /root/scripts/Cameraserver/Failed_Details/motion_device.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/motion_device.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/motion_device.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/motion_device.csv
