...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Cronjob" | head -n1 | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - All Required Jobs Are Added"

else

echo "$Machine_Name - Required Cronjobs Are Not Added"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Cronjob,Failed" >> /root/scripts/Cameraserver/Failed_Details/cronjob.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/cronjob.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/cronjob.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/cronjob.csv
