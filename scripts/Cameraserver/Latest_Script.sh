...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "atest Scripts" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - All The Scipts Are Latest"

else

echo "$Machine_Name - Doen't Have Latest Script"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Latest_Script,Failed" >> /root/scripts/Cameraserver/Failed_Details/Latest_Script.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Latest_Script.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Latest_Script.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Latest_Script.csv
