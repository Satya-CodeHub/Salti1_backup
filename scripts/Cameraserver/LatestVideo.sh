...
####
if grep -q "Latest Video" $Machine_List/$Machine_Name; then
    Alert=$(grep -i "Latest Video" $Machine_List/$Machine_Name | cut -d"-" -f2 | tr -d ' ')
...
echo "$Machine_ID,Latest_Video,Failed" >> /root/scripts/Cameraserver/Failed_Details/Latest_Video.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Latest_Video.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Latest_Video.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Latest_Video.csv
