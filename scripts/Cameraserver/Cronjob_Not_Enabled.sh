...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Cronjob Not Enabled" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Cronjobs Are Enabled"

else

echo "$Machine_Name - Cronjobs Are Not Enabled"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Cronjob_Enable,Failed" >> /root/scripts/Cameraserver/Failed_Details/Cronjob_Not_Enabled.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Cronjob_Not_Enabled.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Cronjob_Not_Enabled.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Cronjob_Not_Enabled.csv
