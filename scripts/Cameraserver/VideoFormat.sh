...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "False_VideoFormat" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - False_VideoFormat not found"

else

echo "$Machine_Name - False_VideoFormat found"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,False_VideoFormat,Failed" >> /root/scripts/Cameraserver/Failed_Details/False_VideoFormat.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/False_VideoFormat.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/False_VideoFormat.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/False_VideoFormat.csv
