...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Motion package" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Motion package installed"

else

echo "$Machine_Name - Motion package not installed"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Motion_package,Failed" >> /root/scripts/Cameraserver/Failed_Details/Motion_package.csv
echo "$Machine_ID,Motion_package,Failed" >> /root/scripts/Cameraserver/Failed_Details/tryMotion_package.csv
fi
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Motion_package.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Motion_package.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Motion_package.csv
