...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Software Wacthdog" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Software Watchdog is Latest"

else

echo "$Machine_Name - Software Watchdog is Not Latest"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Software_Watchdog,Failed" >> /root/scripts/Cameraserver/Failed_Details/software_watchdog.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/software_watchdog.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/software_watchdog.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/software_watchdog.csv
