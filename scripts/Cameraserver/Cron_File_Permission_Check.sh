...
Alert=$(cat $Machine_List/$Machine_Name | grep -i "Cron File Permission Check" | cut -d"-" -f2 | tr -d ' ')

if [ "$Alert" == "Success" ];then

echo "$Machine_Name - Cron File Permission Proper"

else

echo "$Machine_Name - Cron File Permission Not Proper"

Machine_ID=$(ls $Machine_List/$Machine_Name | cut -d"_" -f3)

echo "$Machine_ID,Cron_Permission,Failed" >> /root/scripts/Cameraserver/Failed_Details/Cron_File_Permission.csv
...
if [ -e /root/scripts/Cameraserver/Failed_Details/Cron_File_Permission.csv ]
then
...
done < /root/scripts/Cameraserver/Failed_Details/Cron_File_Permission.csv

rm -rf /root/scripts/Cameraserver/Failed_Details/Cron_File_Permission.csv
