...
BLE_Status="select count(Alert_Name) as BLE_Status from csalerts where Alert_Name='BLE_Status' and Date_Created >= DATE_SUB(NOW(),INTERVAL 50 minute);"
...
BLE_Status=$(cat $TEMPORARY_DIR/CS_Status.csv | grep -i "BLE_Status" -A1 | tail -n1)
...
echo '<tr><td bgcolor=#ffffff><FONT COLOR=black FACE="Merriweather"><b>BLE_Status</b></td><td align="center" valign="center" bgcolor=#ffffff>'$BLE_Status'</td></tr>' >> $TEMPORARY_DIR/csalert.html
