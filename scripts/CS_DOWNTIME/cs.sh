...
sudo find /root/scripts/Cameraserver/Machine_Details -type f -mmin -720 | awk -F/ '{print $NF}' > ${Master_Path}/Latest_CS
...
sudo sort -t$'\t' -k2.9,2.12n -k2.4,2.6M -k2.1,2.2n  -k1,1n /root/scripts/CS_DOWNTIME/formatted_final_CSdown


ALERT_DIR="/home/satyanarayanbiswal/vpnlogin/"

vlo_file="$ALERT_DIR/vlo"

# Copy the content of the file named `klo` to a new file named `vlo`
cp "$ALERT_DIR/klo" "$ALERT_DIR/vlo"
...
cp "$ALERT_DIR/klo" "$ALERT_DIR/klo.html"
sed -i '1i <html><body><table border=5>' "$ALERT_DIR/klo.html"
sed -n -i 'p;1a <th bgcolor=navy><FONT COLOR=white FACE="Merriweather"SIZE=2>Machine <th bgcolor=navy><FONT COLOR=white FACE="Merriweather"SIZE=2>Last_Accessed <th bgcolor=navy><FONT COLOR=white FACE="Merriweather"SIZE=2>City <th bgcolor=navy><FONT COLOR=white FACE="Merriweather"SIZE=2>Location </td></tr><tr>' "$ALERT_DIR/klo.html"
sed -i '$a </table></body></html>' "$ALERT_DIR/klo.html"
